...
const STRONG_CODE =
  /^(RB|BOX|RACK|AM|RV|FT|BUMPER|BRACK|BULL|BAR|EST|BL|TB|LOCK|SYSTEM|CASETA|CANOPY)/i
...
/** Devuelve solo el texto y los candidatos de producto. */
export async function parsePdfText(buffer) {
  const data = await pdfParse(buffer)
  return {
    pages: data.numpages,
    products: extractProductsFromText(data.text || ''),
  }
}
